...
public class BinaryTree {
	...
	public void aftorder() {
		...
		afterTraverseRecursion(root);
		System.out.println();
	}

	private void afterTraverseRecursion(Node<Integer> node) {
		if (node.getLeftChild() != null)
			afterTraverseRecursion(node.getLeftChild());

		if (node.getRightChild() != null)
			afterTraverseRecursion(node.getRightChild());
	...
	public void postorderNoRecursion() {
		...
		Node<Integer> rNode = null;//记录上一个访问的节点
		Node<Integer> current = root;
		LinkedList<Node<Integer>> stack = new LinkedList<Node<Integer>>();
		while (current != null || !stack.isEmpty()) {
			while (current != null) {
