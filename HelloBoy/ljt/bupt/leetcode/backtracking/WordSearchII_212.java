...
public class WordSearchII_212 {
	...
	public static void main(String[] args) {
		char[][] board = { { 'o', 'a', 'a', 'n' }, { 'e', 't', 'a', 'e' }, { 'i', 'h', 'k', 'r' },
				{ 'i', 'f', 'l', 'v' } };
		String[] word = { "oath", "pea", "eat", "rain","eaa" };
		List<String> re = findWords(board, word);
		for (String l : re)
			System.out.println(l);
	}
//  超时，题目提示需要使用trie树来做
	private static List<String> findWords(char[][] board, String[] word) {
	...
	public static boolean helper(char[][] board, boolean[][] visit, String word, int i, int j, int pos) {
		...
		visit[i][j] = true;
		boolean res = helper(board, visit, word, i + 1, j, pos + 1) || helper(board, visit, word, i - 1, j, pos + 1)
				|| helper(board, visit, word, i, j - 1, pos + 1) || helper(board, visit, word, i, j + 1, pos + 1);
		visit[i][j] = false;
		return res;
	}

	// 我自己写的dfs
	// 经常会遇到需要跳出整个回溯的问题，现在有种做法就是设一个全局boolean变量，找到一个置为true。然后下面就对这个boolean进行判断剪枝，详细情况见exist1
	...
	private static boolean DFS(char[][] board, int row, int col, boolean[][] used, char[] words, int wordsIndex) {
		...
		for (int i = 0; i < 4; i++) {
			if (row + goes[i][0] < board.length && row + goes[i][0] >= 0 && col + goes[i][1] < board[0].length
					&& col + goes[i][1] >= 0 && wordsIndex + 1 <= words.length
					&& board[row + goes[i][0]][col + goes[i][1]] == words[wordsIndex]
